...
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_entries, list) {
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        mutex_lock(&monitored_entries_lock);
        list_for_each_entry(existing, &monitored_entries, list) {
        ...
        list_add_tail(&entry->list, &monitored_entries);
        mutex_unlock(&monitored_entries_lock);
    ...
    mutex_lock(&monitored_entries_lock);
    list_for_each_entry(entry, &monitored_entries, list) {
